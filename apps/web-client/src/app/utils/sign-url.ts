...
export function sign_data(url) {
  ...
  var ret = wasm.sign_data(ptr0, len0)
 ...
 * @returns {any}
 */
export function sign_url(url, stamp) {
  var ret = wasm.sign_url(addHeapObject(url), addHeapObject(stamp))
  return takeObject(ret)
}
 ...
 * @param {any} url
 * @param {any} stamp
 * @param {any} current_token
 * @returns {any}
 */
export function refresh_token(url, stamp, current_token) {
  var ret = wasm.refresh_token(
    addHeapObject(url),
    addHeapObject(stamp),
    addHeapObject(current_token)
  )
  return takeObject(ret)
}
...
async function init(input) {
  if (typeof input === 'undefined') {
    input = './assets/clientsdk_bg.wasm'
  }
  const imports = {}
  imports.wbg = {}
