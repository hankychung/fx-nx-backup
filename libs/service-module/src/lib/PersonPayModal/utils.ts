...
export const regFenToYuan = (a: number )=>{
    ...
    if(!num){//等于0
        ...
        num1 = num+'';
        num1+='';//转成字符串
        const reg= num1.indexOf('.') >-1 ? /(\d{1,3})(?=(?:\d{3})+\.)/g : /(\d{1,3})(?=(?:\d{3})+$)/g;//千分符的正则
        // console.log(num.indexOf('.')>-1)
        return num1.replace(reg, '$1,')//千分位格式化
    }
}

export const  getResidueTime=(end:number)=> {
    ...
    const hour = parseInt(`${(totalSeconds / 3600) % 24}`).toString().padStart(2, "0"); //时
    const minute = parseInt(`${(totalSeconds / 60) % 60}`).toString().padStart(2, "0"); //分
    const second = parseInt(`${totalSeconds % 60}`).toString().padStart(2, "0"); //秒
